...
int main(int argc, char **argv) {
    ...
    // 计算每个进程每个文件的6个波段的均值
    std::vector<double> local_band_min(6, std::numeric_limits<double>::infinity());
    for (const auto& file : local_files) {
        std::vector<double> band_means = calculate_band_means(file);
        for (int i = 0; i < 6; ++i) {
            local_band_min[i] = std::min(local_band_min[i], band_means[i]);
        }
    }

    // 根进程通过点对点通信从其他进程收集局部最小值
    if (rank == 0) {
        // 根进程自己有最小值
        std::vector<double> global_band_min = local_band_min;

        // 从其他进程收集最小值
        for (int i = 1; i < size; ++i) {
            MPI_Recv(global_band_min.data(), 6, MPI_DOUBLE, i, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
            // 更新全局最小值
            for (int j = 0; j < 6; ++j) {
                global_band_min[j] = std::min(global_band_min[j], global_band_min[j]);
            }
        }

        // 输出各波段最小值
        for (int i = 0; i < 6; ++i) {
            std::cout << "Min of band " << i + 1 << ": " << global_band_min[i] << std::endl;
        }
    } else {
        // 其他进程将自己的局部最小值发送给根进程
        MPI_Send(local_band_min.data(), 6, MPI_DOUBLE, 0, 0, MPI_COMM_WORLD);
    }
