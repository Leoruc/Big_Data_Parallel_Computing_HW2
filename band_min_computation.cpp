...
int main(int argc, char **argv) {
    ...
    for (const auto& file : local_files) {
        ...
        for (int i = 0; i < 6; ++i) {
            local_band_means[i] = std::min(local_band_means[i], band_means[i]);
        }
    }

    // 使用 MPI_Reduce 计算全局均值
    std::vector<double> global_band_min(6, 0.0);
    MPI_Reduce(local_band_means.data(), global_band_min.data(), 6, MPI_DOUBLE, MPI_MIN, 0, MPI_COMM_WORLD);
