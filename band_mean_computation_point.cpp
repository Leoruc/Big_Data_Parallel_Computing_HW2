...
int main(int argc, char **argv) {
    ...
    if (rank == 0) {
        // 根进程计算自己的均值并接收其他进程的均值
        for (int i = 0; i < 6; ++i) {
            global_band_means[i] = local_band_means[i];  // 首先把自己的均值放入全局数组
        }

        // 收集其他进程的均值
        for (int i = 1; i < size; ++i) {
            MPI_Recv(&global_band_means[0], 6, MPI_DOUBLE, i, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
            // 将其他进程的均值加到根进程的均值上
            for (int j = 0; j < 6; ++j) {
                global_band_means[j] += local_band_means[j];
            }
        }
    } else {
        // 其他进程将均值发送给根进程
        MPI_Send(&local_band_means[0], 6, MPI_DOUBLE, 0, 0, MPI_COMM_WORLD);
    }
